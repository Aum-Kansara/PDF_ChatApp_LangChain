...
def main():
    ...
    user_quetion=st.text_input("Ask a question about your documents :")
    if user_quetion:
        handleUserQue(user_quetion)

    st.write(user_template.replace("{{MSG}}","Hello Robo"),unsafe_allow_html=True)
    st.write(bot_template.replace("{{MSG}}","Hello Human"),unsafe_allow_html=True)
    with st.sidebar:
        st.subheader("Your Documents")
        pdf_docs=st.file_uploader("Enter your PDFs here and click on process",accept_multiple_files=True)
        if st.button("Process"):
            with st.spinner("Processing"):
                ...
                # Create Conversation
                st.session_state.conversation=getConversationalChain(vector_store)
